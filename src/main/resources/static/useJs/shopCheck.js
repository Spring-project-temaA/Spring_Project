...
var checkList = [false, false, false, false, false, false, false, false, false, false, false, false];
var adrrCnt = 0;
...
// owner Buisness
$(function () {
    $("#ownerBuisness").on("blur keyup", function () {
        $(this).val($(this).val().replace(/[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]|[ \{\}\[\]\/?.,;:|\)*~`!^\-+┼<>@\#$%&\'\"\\\(\=]/g, ''));
    });
});
...
// 사업자 번호 유효성 검사
function checkBuisness() {
    var ownerBis = $('#ownerBuisness');
    var value = $('#ownerBuisness').val();
    var valueMap = value.replace(/-/gi, '').split('').map(function (item) {
        return parseInt(item, 10);
    });
    if (value == '') {
        checkList[11] = false;
        $(".checkBusiness_not").css("display", "none");
        $(".checkBusiness").css("display", "inline-block");
        $(".checkBusiness_ok").css("display", "none");
    } else if (valueMap.length != 10) {
        checkList[11] = false;
        $(".checkBusiness_not").css("display", "inline-block");
        $(".checkBusiness").css("display", "none");
        $(".checkBusiness_ok").css("display", "none");
    } else if (valueMap.length === 10) {
        ...
        if (Math.floor(valueMap[9]) === ((10 - (checkSum % 10)) % 10) === false) {
            checkList[11] = false;
            $(".checkBusiness_not").css("display", "inline-block");
            $(".checkBusiness").css("display", "none");
            $(".checkBusiness_ok").css("display", "none");
            ...
            $(".checkBusiness").css("display", "none");
            $(".checkBusiness_not").css("display", "none");
            $(".checkBusiness_ok").css("display", "inline-block");
        }
    }
}
// function checkBuisness() {
//     var checkValue = $('#ownerBuisness').val();
//
//     if (checkValue == '') {
//         $('.checkBusiness_no').css("display", "inline-block");
//         $('.checkBusiness_ok').css("display", "none");
...
function checkAll() {
    ...
    // 값이 비었는지 확인하기 위한 리스트
    let checkVal = [$('#shopId').val(), $("#shopPw").val(), $("#shopPw2").val(), $('#shopName').val(), $('#shopTel').val(), $("#ownerName").val(), $("#ownerPh").val(), $("#ownerMail").val(), $(".ownerGen").val(), $('#shopAddrNum').val(), $('#ownerWord').val(), $("#ownerBuisness").val()]

    // 값이 없을때를 알리기 위한 리스트
    let checkSpan = [$('.checkId'), $('.checkPw'), $('.checkPw2'), $('.checkShopName'), $('.checkTel'), $('.checkName'), $('.checkPh'), $('.checkMail'), $('.checkGen'), $('.checkAddr'), $('.checkWord'), $('.checkBusiness')];
